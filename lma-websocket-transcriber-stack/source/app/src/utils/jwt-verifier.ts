...
const cognitoJwtVerifier = USERPOOL_ID 
    ? CognitoJwtVerifier.create({ userPoolId: USERPOOL_ID })
    : null;
...
export const jwtVerifier = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    // Skip authentication if disabled or no verifier configured
    if (DISABLE_AUTH || !cognitoJwtVerifier) {
        request.log.warn(`[AUTH]: [${clientIP}] - Authentication disabled for local development. URI: <${request.url}>`);
        return;
    }
